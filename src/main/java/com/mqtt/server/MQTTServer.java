...
public final class MQTTServer
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		try
		{
			...
			b.group(bossGroup, workerGroup).channel(NioServerSocketChannel.class).handler(new LoggingHandler(LogLevel.INFO)).childHandler(new MQTTServerInitializer(sslCtx))
					.option(ChannelOption.SO_BACKLOG, 128)// 等待队列
					// .option(ChannelOption.SO_REUSEADDR,
					// true)//表示是否允许重用服务器所绑定的地址
			...
			System.out.println("ITDAP MQTT 启动成功,端口号:" + PORT);
